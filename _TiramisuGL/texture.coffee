...
StaticTexture2D.create_Image = (url, param) ->
...
StaticTextureCube.create_Image = (urlsArray, param) ->
  return Promise.all([
...
StaticTextureCube.create_Data = (dataArray, param) ->
  textureCube = createTextureCubeHelper(true, dataArray, param)
  if not textureCube
    throw new Error("Create StaticTexture2D ERR.")
  return textureCube

StaticTextureCube.create_Size = (width, height, param) ->
  textureCube = createTextureCubeHelper(false, width, height, param)
  if not textureCube
    throw new Error("Create StaticTexture2D ERR.")
  return textureCube

StaticTextureCube.destory = () ->
